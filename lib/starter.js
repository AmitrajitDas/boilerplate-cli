...
const starter = async (project) => {
  ...
  try {
    ...
    await animate("[ 3 / 3 ] linking node_modules...", 1000);
    console.log(
      "\u001b[2m──────────────────────────────────────────\u001b[22m"
    );
    ...
    await installNodeModules(project, spinner);

    isUpdated && (await updateNodeModules(project, spinner));
    isDeduped && (await dedupeNodeModules(project, spinner));

    await postInstallScripts(project, template, spinner);
    await createGitignore(project, spinner);
    await initGit(project);

    await succeedConsole(template, spinner);
  } catch (err) {
    await failConsole(err, spinner);
  }
};
